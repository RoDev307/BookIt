...
<body class="bg-slate-50 font-sans antialiased flex items-center justify-center min-h-screen">
    ...
    <div class="max-w-md w-full mx-4 bg-white rounded-3xl shadow-xl border border-slate-100 p-8 text-center">
        ...
        <p class="text-sm text-slate-500 mt-2">Tu espacio ha sido bloqueado en la agenda del comercio. ¡Te esperamos!
        </p>
        ...
        <div class="mt-4">
            <a href="{{ route('appointments.pdf', ['fecha' => session('fecha'), 'hora' => session('hora')]) }}"
                class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm py-3.5 rounded-xl shadow-sm transition-all text-center block">
        ...
        <div class="mt-8">
            <a href="/negocios"
                class="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold text-sm py-3.5 rounded-xl shadow-sm transition-all text-center block">
                Volver al Catálogo
            </a>
        </div>
    </div>
